...
class BaseDelegatorTest extends HermodBaseTest {
	...
	@InjectMocks
	private BaseDelegatorImpl taskExecutor;

	@Test
	void testFormatDelegatorException(TestInfo testInfo) throws Exception {
		try {
			enrichTestInfo(testInfo, EXP_NOT_NULL.toString());
			logger.debug(getStartTestLog());

			DelegatorException result = taskExecutor.formatDelegatorException(ERROR_MESSAGE, ERROR_CODE);
			...
			result = taskExecutor.formatDelegatorException(ERROR_MESSAGE, ERROR_CAUSE);
			...
			result = taskExecutor.formatDelegatorException(ERROR_MESSAGE);
			assertNotNull(result, getEndTestLog(TEST_AND_OK));
			assertNotNull(result.getMessage(), getEndTestLog(TEST_AND_OK));
			...
			result = taskExecutor.formatDelegatorException(ERROR_TYPE, ERROR_CAUSE);
			assertNotNull(result, getEndTestLog(TEST_AND_OK));
			assertNotNull(result.getMessage(), getEndTestLog(TEST_AND_OK));
			...
			result = taskExecutor.formatDelegatorException(ERROR_TYPE);
			assertNotNull(result, getEndTestLog(TEST_AND_OK));
			assertNotNull(result.getMessage(), getEndTestLog(TEST_AND_OK));
			assertNotNull(result.getCode(), getEndTestLog(TEST_AND_OK));
	...
	@Test
	void testIncludeAndFormatIntoDelegatorException(TestInfo testInfo) throws Exception {
		try {
			enrichTestInfo(testInfo, EXP_NOT_NULL.toString());
			logger.debug(getStartTestLog());

			DelegatorException result = taskExecutor.includeAndFormatIntoDelegatorException(ERROR_THROWABLE, ERROR_CODE);
			assertNotNull(result, getEndTestLogKO());
			assertNotNull(result.getMessage(), getEndTestLogKO());
			...
			result = taskExecutor.includeAndFormatIntoDelegatorException(ERROR_THROWABLE);
			assertNotNull(result, getEndTestLog(TEST_AND_OK));
			assertNotNull(result.getMessage(), getEndTestLog(TEST_AND_OK));
			assertNotNull(result.getCause(), getEndTestLog(TEST_AND_OK));
